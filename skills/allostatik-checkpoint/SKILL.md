---
name: allostatik-checkpoint
description: Run Allostatik's mid-session capture ritual — checkpoints, side notes, observation candidates, decision locks. Use when the user says "checkpoint" or "side note" in a project with an allostatik/ folder, when a choice gets settled in conversation, or when a process friction or pattern worth keeping shows up mid-session. Capture in the moment; don't wait for the close.
---

# allostatik-checkpoint — capture the loop while it's running

You are capturing process signal mid-session in an Allostatik project. The
routine is NOT in this skill — it is the **Capturing observations and
decisions** section of the project's own `allostatik/workflow.md` (and the
...
1. Identify what fired: checkpoint / side note / settled decision / emerging
   pattern.
2. Read `allostatik/workflow.md` **Capturing observations and decisions** (and
