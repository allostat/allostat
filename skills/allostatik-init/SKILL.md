---
name: allostatik-init
description: Set up Allostatik in a project — new or existing. Use when a user asks to install, set up, adopt, or migrate a project onto Allostatik, or asks to "run" the Allostatik repo. Places the real template files, then hands off to workflow.md's first-run or migrate routine.
---

# allostatik-init — the conversational on-ramp

You are helping a person set Allostatik up in their project. Allostatik is not a
...
2. **The project's `allostatik/` folder is reserved** for that project's own
...
2. **Place the files** — in preference order:
   ...
   - No shell (e.g. a files-only surface): have the person get the repo (clone
     or Download ZIP) and copy `templates/project-boilerplate/allostatik/` into
     the project root, plus the `CLAUDE.md` handling per rule below. Verify
     yourself afterward: list `allostatik/` and confirm the eight core files
...
The person ends with: real (not reconstructed) files in `allostatik/`, filled
