...
4. **Deploy the pointer.** Put the short "this is an Allostat project — load the `allostat/` files and follow `workflow.md`" block into the project's instructions (the Project Instructions field, or `CLAUDE.md` on Claude Code) so every later conversation picks the files up.
...
{
  cd <PROJECT-ROOT> && \
  for f in allostat/project-instructions.md allostat/plan.md \
           allostat/workflow.md allostat/decisions.md \
           allostat/observations.md <OTHER-CANONICAL-FILES> ; do
    echo "===== $f ====="; cat "$f"; echo ""
  done
} 2>&1 | <CLIPBOARD-PIPE>
