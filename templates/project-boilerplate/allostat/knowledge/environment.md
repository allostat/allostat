...
> **Never put secrets here.** Env-var *names*, ports, and URLs are fine; actual keys, tokens, passwords, and credentials are not — these files are meant to be committed. Point at where a secret lives (a `.env` file, a secrets manager), never paste its value. (Applies to every `allostat/` file, but the risk is highest here.)
