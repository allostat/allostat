...
Canonical template source: allostatik/templates/user-scope/global_preferences_template.md
...
## The minimal add — make Allostatik projects recognizable

This is the only Layer-1 change Allostatik itself needs. Add this block to your
account-level instructions so the AI recognizes an Allostatik project when it
opens one:

> Some of my projects use a layered context system: canonical files (an
> `allostatik/` folder, with a `CLAUDE.md` listing what to load) that are the
...
follow everywhere. It's optional — Allostatik works with just the block above —
...
rules live in that project's `allostatik/project-instructions.md` (L2); machine
and tool details live in `allostatik/knowledge/environment.md` (L3).
...
*Why so lean? Allostatik treats Layer 1 as optional and minimal by default — the
